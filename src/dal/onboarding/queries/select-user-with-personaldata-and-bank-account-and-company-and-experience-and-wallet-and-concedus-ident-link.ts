...
export const selectUserWithPersonalDataAndBankAccountAndCompanyAndExperienceAndWalletAndConcedusIdentLink =
  async () => {
    ...
    const { data, error } = await supabase
      .from('user')
      .select(
        '*, personal_data(*), bank_account(*), company(*), experience(*), wallet(*), concedus_ident_link(*)',
      )
      .eq('concedus_ident_link.kyc', 'WEBID')
      // .eq('bank_account.status', 'verified')
      .order('created_at', {
        ascending: false,
        referencedTable: 'concedus_ident_link',
      })
      .single()
